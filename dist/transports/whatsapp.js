...
export class WhatsAppProvider {
    ...
    constructor(config, auth) {
        ...
        this.authPath = config.authPath || path.join(process.env.HOME || process.env.USERPROFILE || ".", ".pi", "msg-bridge-whatsapp-auth");
    ...
    async handleMessage(msg) {
        ...
        const isAuthorized = await this.auth.checkAuthorization(userId, chatId, pushName, isGroupChat, wasMentioned, sendMessageToUser, this.type);
        // Handle admin commands and challenge codes in DM
        if (!isGroupChat && (text.startsWith("/") || text.match(/^\d{6}$/))) {
            const handled = await this.auth.handleAdminCommand(text, chatId, userId, async (text) => await this.sendMessage(chatId, text), this.type);
            if (handled) {
                return;
            }
        }
