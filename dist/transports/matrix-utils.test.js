...
import { escapeHtml, extractUsername, formatForMatrix, shouldSkipEvent, stripBotMention, wasBotMentioned, } from "./matrix-utils.js";
// ─── escapeHtml ───────────────────────────────────────────────
describe("escapeHtml", () => {
...
describe("shouldSkipEvent", () => {
    ...
    it("skips edits (m.new_content present)", () => {
        expect(shouldSkipEvent(makeEvent({ content: { msgtype: "m.text", body: "edited", "m.new_content": { body: "new" } } }), botUserId, connectedAt, joinedRooms, "!room1:matrix.org")).toBe("edit");
    });
...
describe("formatForMatrix properties", () => {
    ...
    it("formattedBody is undefined when no markdown chars present", () => {
        ...
        fc.assert(fc.property(noMarkdown, (text) => {
            const result = formatForMatrix(text);
            expect(result.formattedBody).toBeUndefined();
        }));
    });
...
describe("stripBotMention properties", () => {
    it("result never contains the bot MXID (verification)", () => {
        ...
        fc.assert(fc.property(mxid, fc.string(), (botId, prefix) => {
...
//# sourceMappingURL=matrix-utils.test.js.map