...
export class ChallengeAuth {
    ...
    channelAuth = new Map();
    blockedUsers = new Map(); // userId -> unblock timestamp
    adminUserId;
    constructor(onShowCode, onNotify, onSendMessage, onSaveAuth) {
        this.onShowCode = onShowCode;
        this.onNotify = onNotify;
        this.onSendMessage = onSendMessage;
        this.onSaveAuth = onSaveAuth;
    }
    /**
     ...
     */
    async checkAuthorization(userId, chatId, username, isGroupChat, wasMentioned, sendMessage, transport) {
     ...
     */
    async initiateChallenge(userId, chatId, username) {
        ...
        if (this.onSendMessage) {
            try {
                await this.onSendMessage(chatId, "🔐 Please enter the 6-digit code provided by the bot admin.\n⏱️ Expires in 2 minutes.");
            }
            catch (_err) {
                // Ignore send errors
            }
     ...
     */
    async handleAdminCommand(text, _chatId, userId, sendMessage, transport) {
     ...
     */
    async validateChallenge(userId, code, sendMessage) {
        ...
        await sendMessage(`❌ Wrong code. ${3 - challenge.attempts} attempts remaining.`);
        return true;
    }
    /**
...
//# sourceMappingURL=challenge-auth.js.map